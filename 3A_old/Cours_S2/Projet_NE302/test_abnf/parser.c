...
/* ---- ETAPE 2 ---------------------------------- */
	...
	#if DEBUG_ALGO_OUT
		if(res) printf(GRN); else printf(RED);
		printf("Sortie d'algo:\n"NC);
		printf( "str:%.20s\nmem:%.20s\n",str,mem);
	#endif
	return res;
}
...
int construire(char *module, node* parent) {
    ...
    String_View sv;
		...
		cacheAddUser(sv);
			...
			printf(CYN"contruire("NC"%s"CYN") Rule is :"SV_Fmt"\n",module,SV_Arg(sv));
			printf(YEL"mem:"NC"%.20s\n",mem);
		#endif
	...
	#if DEBUG_CONSTRUIRE_OUT
			printf(MAG"contruire("NC"%s"MAG") is ",module);
			truth(valid);
		#endif
