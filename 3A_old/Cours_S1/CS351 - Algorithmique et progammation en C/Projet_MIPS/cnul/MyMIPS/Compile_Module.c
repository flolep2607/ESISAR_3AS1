...
uint8_t str2reg(char* str){
	...
	else sscanf(str,"$%d",&regnum); if (regnum>31) regnum = 0xFF;
...
int inst2hex(char * name, char * arg1, char * arg2, char * arg3){
	...
	*/
	...
	/* ADD rd,rs,rt */
	if(strcmp("ADD",name)==0){//OK DONE
		category = 'R';
		OP = 0b100000;
	} else
	if(!strcmp("ADDI",name)){//OK  DONE
		SPECIAL = 0b001000;
		category = 'I';
	} else
	if(!strcmp("SUB",name)){//OK
		category = 'R';
		OP = 0b100010;
	} else
	if(!strcmp("MULT",name)){
	...
	if(!strcmp("AND",name)){//OK DONE
		category = 'R';
		OP = 0b100100;
	} else
	if(!strcmp("OR",name)){//OK
		category = 'R';
		OP = 0b100101;
	} else
	if(!strcmp("XOR",name)){//OK
		category = 'R';
		OP = 0b100110;
	} else
	...
	if(!strcmp("SLL",name)){//OK
		// SPECIAL = 0b000000;
		category = 'S';
	} else
	if(!strcmp("SRL",name)){//OK
		OP = 0b000010;
		category = 'S';
	} else
	
	if(!strcmp("SLT",name)){//OK
		category = 'R';
		OP = 0b101010;
	} else
	...
	if(!strcmp("LW",name)){
		SPECIAL = 0b100011;
		rt = str2reg(arg1);
		sscanf(arg2,"%hud(%s)",&imm,arg3);//triste :(
	...
	if(!strcmp("MFHI",name)){//OK
		category = 'R';
		OP = 0b010000;
	} else
	if(!strcmp("MFLO",name)){//OK
		category = 0;
		OP = 0b010010;
	} else
	
	if(!strcmp("BEQ",name)){//OK DONE
		SPECIAL = 0b000100;
		category = 4;
	} else
	if(!strcmp("BNE",name)){//OK
		SPECIAL = 0b000101;
		category = 4;
	} else
	if(!strcmp("BGTZ",name)){//OK DONE
		SPECIAL = 0b000111;
		category = 5;
	} else
	if(!strcmp("BLEZ",name)){//OK DONEMFHI
		SPECIAL = 0b000110;
		category = 5;
	} else
	if(!strcmp("J",name)){
...
void compile_segment(char *nomfichier) {
	...
	char line_cpy[TAILLE_LIGNE_MAX];
	...
	while(!feof(fic)) {
		...
		*name='\0'; *arg1='\0'; *arg2='\0'; *arg3='\0';
		...
		/* fill name and argument strings */
		strcpy(line_cpy, line);
		line_decomposer(line_cpy, name, arg1, arg2, arg3);
		...
		if (line[i] = '\0'){
			//empty line (or only whitespaces)
			error = UNDEFINED;
		} else {
		...
		/* do the stuff */
		hex = inst2hex(name, arg1, arg2, arg3);
		
		/* si hex est non nul ou l'operation est NOP */
		if( !strcmp(name,"NOP") || hex){
			fprintf(fout,"%08X\n",hex);
		}
