...
/**
\update: si on prends en compte le travail de préprocessing qui à été prévu mais pas réalisé
		 cette fonction pourrait être réduite à la ligne [ sscanf(str,"$%hhd",&regnum); ]
	...
	*/
	...
	switch(category){
		...
		case 'S': // INST rd,rt,sa
			...
			sscanf(arg3,"%hhud",&sa);
			break;
		case 'Y': // INST, rt, offset(base) (LW,SW)
			rt = str2reg(arg1);
			sscanf(arg2,"%hud",&imm);
			rs = str2reg(arg3);
			break;
		...
		*name='\0'; *arg1='\0'; *arg2='\0'; *arg3='\0';
		...
		if (index != -1) {
			...
			if (PAS && !skip) {
				...
				printf("%s\n",line);
				printf("Hexadecimal instruction code:\t");
				printf("%x\n\n",hex);
				puts("*** [n]: got to next instruction; [s]: skip to execution ***");
				// scanf("%c\n",&c);
				c = getchar();
				if (c=='s') skip = 1;
				while (getchar() != '\n');
